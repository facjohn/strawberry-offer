...
const CACHE_VERSION = 'hn-v54';   // v54: 알포도 2종(8kg 800g×10·9kg 1kg×9) 복구 — OCEAN 시트에 존재
// v53: 포도 스펙 카테고리 개편 — 벌크작업(…수)·팩작업·기프트, '알포도' 폐지(8kg/9kg 숨김), 4.05kg 450g×9팩, 옛 라벨 기록 매칭 유지
// v52: 스펙별 배 사이즈 구성 메모(스펙 선택·카드) + 미국 15kg·7.5kg·기타 스펙도 수수료 5% 가산
// v51: 미국 수수료 5%(원물×5%)를 직접 입력한 작업비 위에도 항상 가산 + 카드/원가내역에 수수료 표시 (시트 =작업비+원물×5% 와 동일)
// v50: 요약 카드 재디자인(밝은 카드·기본 한 줄 요약바·스펙별 오퍼가/수량/마진, 운임·만적·실림율 제거) + 컨테이너/운송 칸 한 줄 전체 배치
// (v49)   // v49: iOS 입력칸 포커스 자동확대 차단(좌우 흔들림 수정) + 운임 진실원천 DATA 전용(OCEAN/Air=스펙만, GAS FREIGHT_DATA_ONLY) + 감사 11건 수정 / v48: 요약 히어로 카드 + 산지 커스텀 + 출발항 선택
